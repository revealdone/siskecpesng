...
<template>
  ...
  <AppLayout>
    <div class="p-6 max-w-4xl mx-auto bg-white shadow rounded space-y-6">
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div v-for="field in ['sk_cpns', 'sk_pns', 'sk_golongan', 'sk_jabatan', 'buku_nikah', 'kartu_pegawai', 'kartu_istri']" :key="field">
            ...
            <div v-if="props.pegawai[field]" class="text-xs text-gray-500 mt-1">
              <a :href="`/storage/${props.pegawai[field]}`" target="_blank" class="text-blue-500 underline">Lihat File Lama</a>
            </div>
          </div>

          <!-- Akte -->
          <div v-for="field in ['akte_suami', 'akte_istri', 'akte_anak_1', 'akte_anak_2', 'akte_anak_3', 'akte_anak_4', 'akte_anak_5']" :key="field">
            <label class="block text-sm capitalize">{{ field.replace('_', ' ') }}</label>
            <input type="file" @change="e => handleFileChange(e, field as keyof PegawaiForm)" />
            <div v-if="props.pegawai[field]" class="text-xs text-gray-500 mt-1">
              <a :href="`/storage/${props.pegawai[field]}`" target="_blank" class="text-blue-500 underline">Lihat File Lama</a>
